...
export default function MultiSelectTag (el, customs = {shadow: false, rounded:true}) {
    var element = null
    var options = null
    ...
    function createTag(option) {
        ...
        console.log("add tag value",option.value);
        console.log("arrPokemons",arrPokemons);
        arrPokemons.push({name:option.value});
    ...
    function removeTag(val) {
        ...
        for(let i = 0; i < inputContainer.children.length; i++){
            // console.log("Name = ",arrPokemons[i].name)
            if(arrPokemons[i].name == val){
                console.log("remove tag val",val)
                return arrPokemons.splice(i,1)
            }
        }
    }
    function setValues() {
